...
const manager = JSON.parse(readFileSync(new URL('../packages/dsh-file-manager/package.json', import.meta.url), 'utf8'))
const sources = planUserFilesInstall({
  profileDirectory,
  consumerPackage: resolve(import.meta.dirname, '../packages/dsh-file-manager'),
})
...
const required = manager.peerDependencies[name]
...
function assertSidebar(manifest, source) {
  ...
  if (!semver.satisfies(manifest.version, required, { includePrerelease: true })) {
    throw new Error(`${name}@${manifest.version} does not satisfy manager peer ${required}; select a compatible sidebar before installation`)
  }
}
