...
- In the current implementation, `FileManagerFilesystem` owns directory mutations plus guarded text and byte writes. The Client owns loaded-tree state, non-overlapping directory and source polling lifetimes, generic resource routing, and right-sidebar registrations.
- In the current implementation, Chat path presentation and preview/system routing belong to the independent resource-links plugin. This manager provides metadata resolution and the Files selector without a reverse dependency or Chat listener.
