...
import type { UserFileFilesystem } from '@dsh-external/dsh-user-files'
...
export function registerFileTransfers(ctx: Context, maxUploadBytes: number): void {
  ctx.effect(() => {
    ...
    const unregister = ctx.webServer.register({
      kind: 'exact', path: '/api/file-manager/transfer',
      handler(req, res) {
    ...
    async function transfer(req: IncomingMessage, res: ServerResponse): Promise<void> {
      ...
      try {
        ...
        const url = new URL(req.url!, 'http://file-manager')
    ...
    return async () => {
      unregister()
      lifetime.abort(new Error('File manager unloaded.'))
      await Promise.all(active)
    }
  }, 'file-manager: authenticated streaming transfers')
}
