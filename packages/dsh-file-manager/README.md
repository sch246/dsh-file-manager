...
Download delegates to user-files ^0.1.12. Supported secure-context browsers use bounded parallel ranges and a selected local writer, with black progress beneath the address. Shift-click selects ordinary browser download; browsers without direct writing use that route automatically. The shared provider owns download policy and metadata guards. Custom downloads follow the tree foreground-operation cancellation; ordinary handoff remains browser-owned.
