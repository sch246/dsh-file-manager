...
it('streams exact binary files with original names, authenticates, and refuses overwrite and oversized uploads', async () => {
  ...
  try {
    ...
    const url = (path: string, name?: string): string => `http://127.0.0.1:${address.port}/api/file-manager/transfer?${new URLSearchParams({ sessionId: 'transfer-session', path, ...(name ? { name } : {}) })}`
