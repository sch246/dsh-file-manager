...
it('resolves bounded metadata batches independently without reading content and never downgrades failed trash', async () => {
  ...
  const filesystem = new FileManagerFilesystem(1, 1, trash, 'mv')
  ...
  try {
    ...
    expect(result).toMatchObject([
      ...
      { ok: false, error: { code: 'file-manager/not-found' } },
      { ok: true, value: { path: root, kind: 'directory' } },
      { ok: false, error: { code: 'file-manager/invalid-path' } },
      { ok: true, value: { path } },
    ])
    ...
    await expect(remote.resolveMany({ sessionId, paths: Array(6).fill(path) }, signal))
      .rejects.toMatchObject({ code: 'file-manager/batch-too-large' })
    ...
    expect(new Uint8Array(await readFile(path))).toEqual(Uint8Array.of(0, 255, 0))
