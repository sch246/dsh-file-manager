...
it.skipIf(process.platform !== 'linux')('mounts the Host with provider-owned trash lookup, no browse mkdir, and literal deletion paths', async () => {
  ...
  try {
    ...
    expect(await remote.metadata()).toMatchObject({ trashDirectory: join(trashRoot, 'files') })
    ...
    await mkdir(join(trashRoot, 'info'))
    const held = join(trashRoot, 'files', 'provider-generated-id')
    const original = join(root, 'restored name.txt')
    const record = join(trashRoot, 'info', 'provider-generated-id.trashinfo')
    await writeFile(record, `[Trash Info]\nPath=${encodeURI(original)}\n`)
    await expect(remote.deleteEntry({ ...request, path: held, mode: 'trash', confirmed: false }, signal)).rejects.toMatchObject({ code: 'file-manager/confirmation-required' })
    await remote.restore({ ...request, path: held }, signal)
    expect(await readFile(original, 'utf8')).toBe('recoverable bytes')
    await expect(readFile(record)).rejects.toMatchObject({ code: 'ENOENT' })
    await expect(remote.restore({ ...request, path: held }, AbortSignal.abort())).rejects.toMatchObject({ code: 'gateway/cancelled' })
