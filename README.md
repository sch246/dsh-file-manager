...
- Each tree keeps in-memory directory history. Mouse back/forward buttons (3/4) over its panel and Alt+Left/Right while its non-editable content has focus revisit successfully loaded directories. Address and filter inputs retain their keyboard editing behavior. Failed or superseded reads do not move history; a new directory after going back discards forward entries. Refresh and repeated navigation to the current directory add no entry. Closing or restoring a tree starts fresh history at its loaded directory; browser page history and other panels remain separate.
