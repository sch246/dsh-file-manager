...
Manager uses the browser-safe `@dsh-external/dsh-user-files/transfer` URL helper for the provider-owned `/api/user-files/transfer` endpoint. It retains upload batches, destination selection, cancellation and browser download handoff; the provider owns authentication, streaming and exclusive publication. Download uses the shared provider helper: supported secure-context browsers select a local file, fetch bounded ranges concurrently and write directly to disk with a straight black progress bar below the address. Hold Shift while clicking Download for ordinary browser handoff; unsupported browsers also use handoff. Errors remain in the tree, and navigation or superseding tree operations cancel unfinished custom downloads. Native handoff retains browser-owned progress and cancellation.

Install user-files ^0.1.12 with the Web composition's `connection` and `webServer` services. Preserve the effective old manager `maxUploadBytes` value in the complete user-files config row, then remove that field from manager. The provider default is 10 GiB per file. Migrate any exact reverse-proxy location from `/api/file-manager/transfer` to `/api/user-files/transfer`, retaining upstream headers, cookie policy, matching request limits, disabled request/response buffering and appropriate timeouts. The provider package reference owns the HTTP protocol and streaming limitations.
