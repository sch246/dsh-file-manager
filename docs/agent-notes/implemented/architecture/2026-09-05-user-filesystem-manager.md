...
The file-manager Bundle owns the `fileManager` Typert namespace, right-sidebar Files launcher and tree view, `filesystem` resource source, polling lifetimes, and Chat file-link routing. Its Host Remote uses Node filesystem APIs with service-process permissions and treats a Session cwd only as the first browser location. Regular files enter the central resource-opening service, which owns handler and workbench placement decisions; the tree requests preview or permanent opening beside its own instance.
...
Recoverable removal sends a file, link, or directory to operating-system trash without confirmation and never falls back when trash fails. Permanent removal remains an explicit UI action with one ordinary confirmation. The Host rejects filesystem root and unlinks a symbolic link rather than recursively traversing its target.
