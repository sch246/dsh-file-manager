...
The resource workbench needs one filesystem source with metadata, independent text and byte operations, guarded saving, and external-change observation. Directory operations and filesystem authority need one owner. Chat path presentation and routing need an independent consumer so their policy can evolve without changing filesystem management.

## Decision

The file-manager Bundle owns the `fileManager` Typert namespace, right-sidebar Files launcher and tree view, `filesystem` resource source, and polling lifetimes. Its Host Remote uses Node filesystem APIs with service-process permissions and treats a Session cwd only as the first browser location. Regular files enter the central resource-opening service, which owns handler and workbench placement decisions; the tree persists the visible selected row and requests preview or permanent opening of its canonical resource beside its own instance. The independent resource-links plugin consumes manager metadata and the Files launcher for Chat path presentation and preview/system routing. The manager has no Chat listener or reverse dependency on that consumer.
...
Recoverable removal sends a file, link, or directory to operating-system trash without confirmation and never falls back when trash fails. Each row exposes one Delete action governed by More's Move to trash when deleting preference; unchecked means permanent removal with one ordinary confirmation. Browser storage owns this manager-wide preference because restoring an individual Session tree must not reset the user's current deletion choice. Host deleteMode initializes absent preferences. Trash receives literal paths with glob expansion disabled. The Host rejects filesystem root and unlinks a symbolic link rather than recursively traversing its target.

Directory actions share the first list row so the path input is the sole persistent control. Hover and keyboard focus reveal the actions; touch devices keep them reachable, and container queries progressively move actions into More. More also owns browser-wide hidden-entry and filter-enabled switches. A disabled filter retains its per-tree query while returning every loaded row. Browser preferences remain separate from navigational restoration so an old tree cannot override a newer choice.

Home-trash browsing uses the same xdg-trashdir dependency and WSL detection as the installed trash provider. It returns the Linux home files directory without creating it, aggregating other volumes, reconstructing original names, or restoring items. UUID storage names remain visible as ordinary filenames. Unsupported platforms and missing directories fail explicitly because a plausible platform path does not establish provider ownership.
...
The sidebar persists a v2 tree descriptor with the current root, expanded paths, selection, and filter query. Its restorer accepts v1 navigation but ignores the legacy hidden-entry preference, reconstructing the tree before marking the instance ready. Close confirmation makes no feature mutation; only the authoritative close notification releases tree state and cancels reads. The deletion Remote uses `deleteEntry` because Cordis Service owns the lifecycle method named `remove`.
